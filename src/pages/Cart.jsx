import Header from '../components/Header'
import Footer from '../components/Footer'
import ScrollTop from '../components/ScrollTop'

function Cart() {
  return (
    <>
      
      <Header />
      
    <main>
        
        <section className="breadcrumb-section breadcrumb-bg">
            <div className="container">
                <div className="row">
					<div className="col-lg-12">
                        <div className="breadcrumb-text">
                            <h1 className="title wow fadeInUp" data-wow-delay="0.1s">Shopping Cart</h1>
                            <nav aria-label="breadcrumb" className="breadcrumb-nav wow fadeInUp" data-wow-delay="0.0s">
                                <ul className="breadcrumb listing">
                                    <li className="breadcrumb-item single-list"><a href="/" className="single">Home</a></li>
                                    <li className="breadcrumb-item single-list" aria-current="page"><a href="#"
                                            className="single active">Shopping Cart</a></li>
                                </ul>
                            </nav>
                        </div>						
					</div>
				</div>
            </div>
        </section>
        
        
        <div className="wishlist-product section-padding">
            <div className="container">
                <div className="row">
					<div className="col-lg-12">
                        <div className="table-responsive">
                            <table className="table product-cart-table">
                                <thead>
                                    <tr>
                                        <th>Image</th>
                                        <th>Product Information</th>
                                        <th>Quantity</th>
                                        <th>Total Price</th>
                                        <th>Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td className="cart-img">
                                            <div className="thumb-img">
                                                <img src="/assets/images/gallery/wish-thumb-1.png" alt="img" />
                                            </div>
                                        </td>
                                        <td className="cart-info">
                                            <div className="cart-box">
                                                <a href="/shop-details">
                                                    <p className="cart-pera mb-15">Kelly Bracelet Amchair</p>
                                                </a>
                                                <div className="ratting-section mb-18">
                                                    <div className="all-ratting">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
        
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                    </div>
                                                    <div className="ratting-count">
                                                        <p className="pera">(22)</p>
                                                    </div>
                                                </div>
                                                <p className="cart-pera">$320</p>
                                            </div>
                                        </td>
                                        <td className="cart-qty">
                                            <div className="quantity-section">
                                                <div className="quantity-btn position-relative">
                                                    <input type="text" name="qty" value="01" className="num-count" readonly />
                                                    <button className="increase-num">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="12" height="6"
                                                            viewBox="0 0 12 6" fill="none">
                                                            <path
                                                                d="M0 5.36618C0.0532405 5.553 0.143084 5.71647 0.319443 5.84199C0.618921 6.04924 1.0515 6.05508 1.35098 5.84491C1.40755 5.80404 1.46411 5.76026 1.51735 5.71355C2.97481 4.43501 4.4356 3.15355 5.89306 1.875C5.92633 1.84581 5.95295 1.81078 6.03281 1.75824C6.0561 1.79327 6.06941 1.83705 6.10269 1.86625C7.57346 3.15938 9.04755 4.4496 10.5183 5.74274C10.7779 5.97043 11.0773 6.058 11.4367 5.95875C11.9591 5.8128 12.1721 5.22315 11.8427 4.83784C11.8061 4.79405 11.7628 4.75318 11.7196 4.71232C10.0225 3.2236 8.32881 1.73489 6.63177 0.249093C6.32231 -0.0223789 5.92966 -0.0749219 5.57361 0.106059C5.4871 0.149845 5.41056 0.214063 5.34069 0.275363C3.69356 1.72029 2.04976 3.16522 0.399304 4.60723C0.216289 4.76778 0.0532405 4.93125 0 5.16185C0 5.22899 0 5.29613 0 5.36618Z"
                                                                fill="currentColor" />
                                                        </svg>
                                                    </button>
                                                    <button className="decrease-num">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="12" height="6"
                                                            viewBox="0 0 12 6" fill="none">
                                                            <path
                                                                d="M12 0.633816C11.9468 0.446997 11.8569 0.28353 11.6806 0.158011C11.3811 -0.0492418 10.9485 -0.0550799 10.649 0.155092C10.5925 0.195958 10.5359 0.239744 10.4826 0.286449C9.02519 1.56499 7.5644 2.84645 6.10694 4.125C6.07367 4.15419 6.04705 4.18922 5.96719 4.24176C5.9439 4.20673 5.93059 4.16294 5.89731 4.13375C4.42654 2.84062 2.95245 1.5504 1.48168 0.257257C1.22213 0.0295717 0.922652 -0.0579998 0.563279 0.0412478C0.0408569 0.1872 -0.172105 0.776848 0.157321 1.16216C0.193924 1.20595 0.237181 1.24681 0.280439 1.28768C1.97748 2.7764 3.67119 4.26511 5.36823 5.75091C5.67769 6.02238 6.07034 6.07492 6.42639 5.89394C6.5129 5.85015 6.58944 5.78594 6.65931 5.72464C8.30644 4.27971 9.95024 2.83478 11.6007 1.39277C11.7837 1.23222 11.9468 1.06875 12 0.838149C12 0.771011 12 0.703873 12 0.633816Z"
                                                                fill="currentColor" />
                                                        </svg>
                                                    </button>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="cart-price">
                                            <p className="cart-pera text-center">$320</p>
                                        </td>
                                        <td className="action">
                                            <div className="text-center">
        
                                                <a href="#" className="del-icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="21"
                                                        viewBox="0 0 20 21" fill="none">
                                                        <path
                                                            d="M9.76126 20.9806C8.58921 20.9291 7.11625 21.0958 5.65575 20.9082C4.14925 20.7177 3.06824 19.5157 2.97241 18.0042C2.78074 15.0954 2.58908 12.186 2.39741 9.27591C2.30253 7.90438 2.19424 6.52618 2.11662 5.14989C2.09841 4.8194 1.97382 4.69558 1.65278 4.71082C1.36541 4.72641 1.07733 4.72386 0.790281 4.7032C0.336988 4.66605 0.0331962 4.36412 0.00252952 3.95076C-0.0290955 3.50692 0.237322 3.1707 0.717448 3.09356C0.957043 3.06347 1.19866 3.05232 1.44003 3.06022C2.81237 3.06022 4.18566 3.03641 5.56087 3.06022C6.03142 3.06879 6.18667 2.90973 6.14737 2.45827C6.11989 2.17394 6.13637 1.88711 6.19625 1.60773C6.40612 0.676239 7.21496 0.0447651 8.26242 0.0200014C9.41242 -0.00666715 10.5624 -0.00666715 11.7124 0.0200014C13.0359 0.0523847 13.8639 0.927686 13.8687 2.24492V2.38779C13.8687 3.05451 13.8687 3.05451 14.5165 3.05451L18.7332 3.06308C18.9086 3.057 19.0843 3.06528 19.2584 3.08784C19.7423 3.16594 20.0174 3.48692 19.9992 3.92885C19.9809 4.37079 19.6714 4.67367 19.1903 4.70796C18.9031 4.72546 18.6152 4.72768 18.3278 4.71462C18.0288 4.7032 17.9061 4.82035 17.8889 5.12894C17.7324 7.86628 17.5682 10.603 17.3963 13.3391C17.2957 14.9516 17.2439 16.5717 17.0504 18.1737C16.8424 19.8986 15.5448 20.9596 13.792 20.9806C12.5462 20.9929 11.3013 20.9806 9.76126 20.9806ZM9.28209 4.71748H4.39554C3.78987 4.71748 3.76879 4.71748 3.80233 5.32705C3.91829 7.39863 4.05054 9.47021 4.17608 11.5418C4.3045 13.6762 4.4262 15.8116 4.56516 17.9461C4.57959 18.3168 4.73856 18.6674 5.00846 18.9237C5.27835 19.1801 5.63799 19.322 6.01129 19.3195C8.67674 19.3328 11.3419 19.3328 14.0067 19.3195C14.3714 19.3142 14.7207 19.1724 14.9849 18.9224C15.249 18.6724 15.4086 18.3326 15.4317 17.9708C15.6017 15.57 15.7614 13.1673 15.9109 10.7627C16.0259 8.94445 16.1026 7.12337 16.2492 5.308C16.2856 4.84797 16.1399 4.7813 15.7413 4.76987C13.586 4.70701 11.4316 4.72415 9.27826 4.71748H9.28209ZM9.99988 3.04689C10.5912 3.04689 11.1815 3.04689 11.7718 3.04689C12.0478 3.04689 12.3037 3.04212 12.2654 2.65829C12.2299 2.30493 12.4111 1.77918 11.8418 1.75632C10.6151 1.70679 9.38367 1.70679 8.157 1.75632C7.58871 1.77918 7.77367 2.30588 7.73438 2.65924C7.69604 3.04022 7.95288 3.04879 8.22888 3.04689C8.81921 3.04689 9.40955 3.04689 9.99988 3.04689Z"
                                                            fill="currentColor" />
                                                        <path
                                                            d="M6.13672 11.978C6.13672 10.6932 6.13672 9.40643 6.13672 8.12253C6.13672 7.51296 6.44818 7.14246 6.93789 7.14246C7.4276 7.14246 7.73235 7.52344 7.73235 8.13205C7.73235 10.7183 7.73235 13.3045 7.73235 15.8907C7.73235 16.506 7.4343 16.8803 6.95226 16.8936C6.47022 16.907 6.13672 16.5174 6.13672 15.8812C6.13672 14.5801 6.13672 13.2791 6.13672 11.978Z"
                                                            fill="currentColor" />
                                                        <path
                                                            d="M9.20312 11.9714C9.20312 10.7015 9.20312 9.43155 9.20312 8.16162C9.20312 7.51395 9.50979 7.13678 10.0129 7.1444C10.4998 7.15107 10.8007 7.52538 10.8007 8.1359C10.8007 10.7228 10.8007 13.3093 10.8007 15.8955C10.8007 16.5079 10.4978 16.8832 10.0148 16.8918C9.53183 16.9003 9.20504 16.5108 9.20312 15.8726C9.20312 14.5716 9.20312 13.2725 9.20312 11.9714Z"
                                                            fill="currentColor" />
                                                        <path
                                                            d="M13.8668 12.0686C13.8668 13.3861 13.8668 14.703 13.8668 16.0193C13.8668 16.4565 13.6626 16.7518 13.241 16.867C12.8931 16.9622 12.618 16.8099 12.4101 16.5327C12.3045 16.3724 12.2553 16.1818 12.2702 15.9908C12.2702 13.341 12.2702 10.6904 12.2702 8.04066C12.2702 7.50443 12.595 7.15202 13.057 7.14631C13.5189 7.14059 13.862 7.50633 13.8639 8.07018C13.8706 9.40266 13.8668 10.7361 13.8668 12.0686Z"
                                                            fill="currentColor" />
                                                    </svg>
                                                </a>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td className="cart-img">
                                            <div className="thumb-img">
                                                <img src="/assets/images/gallery/wish-thumb-2.png" alt="img" />
                                            </div>
                                        </td>
                                        <td className="cart-info">
                                            <div className="cart-box">
                                                <a href="/shop-details">
                                                    <p className="cart-pera mb-15">Candelaria Fabric Sofa</p>
                                                </a>
                                                <div className="ratting-section mb-18">
                                                    <div className="all-ratting">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                    </div>
                                                    <div className="ratting-count">
                                                        <p className="pera">(36)</p>
                                                    </div>
                                                </div>
                                                <p className="cart-pera">$600</p>
                                            </div>
                                        </td>
                                        <td className="cart-qty">
                                            <div className="quantity-section">
                                                <div className="quantity-btn position-relative">
                                                    <input type="text" name="qty" value="01" className="num-count" readonly />
                                                    <button className="increase-num">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="12" height="6"
                                                            viewBox="0 0 12 6" fill="none">
                                                            <path
                                                                d="M0 5.36618C0.0532405 5.553 0.143084 5.71647 0.319443 5.84199C0.618921 6.04924 1.0515 6.05508 1.35098 5.84491C1.40755 5.80404 1.46411 5.76026 1.51735 5.71355C2.97481 4.43501 4.4356 3.15355 5.89306 1.875C5.92633 1.84581 5.95295 1.81078 6.03281 1.75824C6.0561 1.79327 6.06941 1.83705 6.10269 1.86625C7.57346 3.15938 9.04755 4.4496 10.5183 5.74274C10.7779 5.97043 11.0773 6.058 11.4367 5.95875C11.9591 5.8128 12.1721 5.22315 11.8427 4.83784C11.8061 4.79405 11.7628 4.75318 11.7196 4.71232C10.0225 3.2236 8.32881 1.73489 6.63177 0.249093C6.32231 -0.0223789 5.92966 -0.0749219 5.57361 0.106059C5.4871 0.149845 5.41056 0.214063 5.34069 0.275363C3.69356 1.72029 2.04976 3.16522 0.399304 4.60723C0.216289 4.76778 0.0532405 4.93125 0 5.16185C0 5.22899 0 5.29613 0 5.36618Z"
                                                                fill="currentColor" />
                                                        </svg>
                                                    </button>
                                                    <button className="decrease-num">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="12" height="6"
                                                            viewBox="0 0 12 6" fill="none">
                                                            <path
                                                                d="M12 0.633816C11.9468 0.446997 11.8569 0.28353 11.6806 0.158011C11.3811 -0.0492418 10.9485 -0.0550799 10.649 0.155092C10.5925 0.195958 10.5359 0.239744 10.4826 0.286449C9.02519 1.56499 7.5644 2.84645 6.10694 4.125C6.07367 4.15419 6.04705 4.18922 5.96719 4.24176C5.9439 4.20673 5.93059 4.16294 5.89731 4.13375C4.42654 2.84062 2.95245 1.5504 1.48168 0.257257C1.22213 0.0295717 0.922652 -0.0579998 0.563279 0.0412478C0.0408569 0.1872 -0.172105 0.776848 0.157321 1.16216C0.193924 1.20595 0.237181 1.24681 0.280439 1.28768C1.97748 2.7764 3.67119 4.26511 5.36823 5.75091C5.67769 6.02238 6.07034 6.07492 6.42639 5.89394C6.5129 5.85015 6.58944 5.78594 6.65931 5.72464C8.30644 4.27971 9.95024 2.83478 11.6007 1.39277C11.7837 1.23222 11.9468 1.06875 12 0.838149C12 0.771011 12 0.703873 12 0.633816Z"
                                                                fill="currentColor" />
                                                        </svg>
                                                    </button>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="cart-price">
                                            <p className="cart-pera text-center">$600</p>
                                        </td>
                                        <td className="action">
                                            <div className="text-center">
        
                                                <a href="#" className="del-icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="21"
                                                        viewBox="0 0 20 21" fill="none">
                                                        <path
                                                            d="M9.76126 20.9806C8.58921 20.9291 7.11625 21.0958 5.65575 20.9082C4.14925 20.7177 3.06824 19.5157 2.97241 18.0042C2.78074 15.0954 2.58908 12.186 2.39741 9.27591C2.30253 7.90438 2.19424 6.52618 2.11662 5.14989C2.09841 4.8194 1.97382 4.69558 1.65278 4.71082C1.36541 4.72641 1.07733 4.72386 0.790281 4.7032C0.336988 4.66605 0.0331962 4.36412 0.00252952 3.95076C-0.0290955 3.50692 0.237322 3.1707 0.717448 3.09356C0.957043 3.06347 1.19866 3.05232 1.44003 3.06022C2.81237 3.06022 4.18566 3.03641 5.56087 3.06022C6.03142 3.06879 6.18667 2.90973 6.14737 2.45827C6.11989 2.17394 6.13637 1.88711 6.19625 1.60773C6.40612 0.676239 7.21496 0.0447651 8.26242 0.0200014C9.41242 -0.00666715 10.5624 -0.00666715 11.7124 0.0200014C13.0359 0.0523847 13.8639 0.927686 13.8687 2.24492V2.38779C13.8687 3.05451 13.8687 3.05451 14.5165 3.05451L18.7332 3.06308C18.9086 3.057 19.0843 3.06528 19.2584 3.08784C19.7423 3.16594 20.0174 3.48692 19.9992 3.92885C19.9809 4.37079 19.6714 4.67367 19.1903 4.70796C18.9031 4.72546 18.6152 4.72768 18.3278 4.71462C18.0288 4.7032 17.9061 4.82035 17.8889 5.12894C17.7324 7.86628 17.5682 10.603 17.3963 13.3391C17.2957 14.9516 17.2439 16.5717 17.0504 18.1737C16.8424 19.8986 15.5448 20.9596 13.792 20.9806C12.5462 20.9929 11.3013 20.9806 9.76126 20.9806ZM9.28209 4.71748H4.39554C3.78987 4.71748 3.76879 4.71748 3.80233 5.32705C3.91829 7.39863 4.05054 9.47021 4.17608 11.5418C4.3045 13.6762 4.4262 15.8116 4.56516 17.9461C4.57959 18.3168 4.73856 18.6674 5.00846 18.9237C5.27835 19.1801 5.63799 19.322 6.01129 19.3195C8.67674 19.3328 11.3419 19.3328 14.0067 19.3195C14.3714 19.3142 14.7207 19.1724 14.9849 18.9224C15.249 18.6724 15.4086 18.3326 15.4317 17.9708C15.6017 15.57 15.7614 13.1673 15.9109 10.7627C16.0259 8.94445 16.1026 7.12337 16.2492 5.308C16.2856 4.84797 16.1399 4.7813 15.7413 4.76987C13.586 4.70701 11.4316 4.72415 9.27826 4.71748H9.28209ZM9.99988 3.04689C10.5912 3.04689 11.1815 3.04689 11.7718 3.04689C12.0478 3.04689 12.3037 3.04212 12.2654 2.65829C12.2299 2.30493 12.4111 1.77918 11.8418 1.75632C10.6151 1.70679 9.38367 1.70679 8.157 1.75632C7.58871 1.77918 7.77367 2.30588 7.73438 2.65924C7.69604 3.04022 7.95288 3.04879 8.22888 3.04689C8.81921 3.04689 9.40955 3.04689 9.99988 3.04689Z"
                                                            fill="currentColor" />
                                                        <path
                                                            d="M6.13672 11.978C6.13672 10.6932 6.13672 9.40643 6.13672 8.12253C6.13672 7.51296 6.44818 7.14246 6.93789 7.14246C7.4276 7.14246 7.73235 7.52344 7.73235 8.13205C7.73235 10.7183 7.73235 13.3045 7.73235 15.8907C7.73235 16.506 7.4343 16.8803 6.95226 16.8936C6.47022 16.907 6.13672 16.5174 6.13672 15.8812C6.13672 14.5801 6.13672 13.2791 6.13672 11.978Z"
                                                            fill="currentColor" />
                                                        <path
                                                            d="M9.20312 11.9714C9.20312 10.7015 9.20312 9.43155 9.20312 8.16162C9.20312 7.51395 9.50979 7.13678 10.0129 7.1444C10.4998 7.15107 10.8007 7.52538 10.8007 8.1359C10.8007 10.7228 10.8007 13.3093 10.8007 15.8955C10.8007 16.5079 10.4978 16.8832 10.0148 16.8918C9.53183 16.9003 9.20504 16.5108 9.20312 15.8726C9.20312 14.5716 9.20312 13.2725 9.20312 11.9714Z"
                                                            fill="currentColor" />
                                                        <path
                                                            d="M13.8668 12.0686C13.8668 13.3861 13.8668 14.703 13.8668 16.0193C13.8668 16.4565 13.6626 16.7518 13.241 16.867C12.8931 16.9622 12.618 16.8099 12.4101 16.5327C12.3045 16.3724 12.2553 16.1818 12.2702 15.9908C12.2702 13.341 12.2702 10.6904 12.2702 8.04066C12.2702 7.50443 12.595 7.15202 13.057 7.14631C13.5189 7.14059 13.862 7.50633 13.8639 8.07018C13.8706 9.40266 13.8668 10.7361 13.8668 12.0686Z"
                                                            fill="currentColor" />
                                                    </svg>
                                                </a>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td className="cart-img">
                                            <div className="thumb-img">
                                                <img src="/assets/images/gallery/wish-thumb-3.png" alt="img" />
                                            </div>
                                        </td>
                                        <td className="cart-info">
                                            <div className="cart-box">
                                                <a href="/shop-details">
                                                    <p className="cart-pera mb-15">Envole Pendant Light</p>
                                                </a>
                                                <div className="ratting-section mb-18">
                                                    <div className="all-ratting">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                    </div>
                                                    <div className="ratting-count">
                                                        <p className="pera">(20)</p>
                                                    </div>
                                                </div>
                                                <p className="cart-pera">$110</p>
                                            </div>
                                        </td>
                                        <td className="cart-qty">
                                            <div className="quantity-section">
                                                <div className="quantity-btn position-relative">
                                                    <input type="text" name="qty" value="01" className="num-count" readonly />
                                                    <button className="increase-num">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="12" height="6"
                                                            viewBox="0 0 12 6" fill="none">
                                                            <path
                                                                d="M0 5.36618C0.0532405 5.553 0.143084 5.71647 0.319443 5.84199C0.618921 6.04924 1.0515 6.05508 1.35098 5.84491C1.40755 5.80404 1.46411 5.76026 1.51735 5.71355C2.97481 4.43501 4.4356 3.15355 5.89306 1.875C5.92633 1.84581 5.95295 1.81078 6.03281 1.75824C6.0561 1.79327 6.06941 1.83705 6.10269 1.86625C7.57346 3.15938 9.04755 4.4496 10.5183 5.74274C10.7779 5.97043 11.0773 6.058 11.4367 5.95875C11.9591 5.8128 12.1721 5.22315 11.8427 4.83784C11.8061 4.79405 11.7628 4.75318 11.7196 4.71232C10.0225 3.2236 8.32881 1.73489 6.63177 0.249093C6.32231 -0.0223789 5.92966 -0.0749219 5.57361 0.106059C5.4871 0.149845 5.41056 0.214063 5.34069 0.275363C3.69356 1.72029 2.04976 3.16522 0.399304 4.60723C0.216289 4.76778 0.0532405 4.93125 0 5.16185C0 5.22899 0 5.29613 0 5.36618Z"
                                                                fill="currentColor" />
                                                        </svg>
                                                    </button>
                                                    <button className="decrease-num">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="12" height="6"
                                                            viewBox="0 0 12 6" fill="none">
                                                            <path
                                                                d="M12 0.633816C11.9468 0.446997 11.8569 0.28353 11.6806 0.158011C11.3811 -0.0492418 10.9485 -0.0550799 10.649 0.155092C10.5925 0.195958 10.5359 0.239744 10.4826 0.286449C9.02519 1.56499 7.5644 2.84645 6.10694 4.125C6.07367 4.15419 6.04705 4.18922 5.96719 4.24176C5.9439 4.20673 5.93059 4.16294 5.89731 4.13375C4.42654 2.84062 2.95245 1.5504 1.48168 0.257257C1.22213 0.0295717 0.922652 -0.0579998 0.563279 0.0412478C0.0408569 0.1872 -0.172105 0.776848 0.157321 1.16216C0.193924 1.20595 0.237181 1.24681 0.280439 1.28768C1.97748 2.7764 3.67119 4.26511 5.36823 5.75091C5.67769 6.02238 6.07034 6.07492 6.42639 5.89394C6.5129 5.85015 6.58944 5.78594 6.65931 5.72464C8.30644 4.27971 9.95024 2.83478 11.6007 1.39277C11.7837 1.23222 11.9468 1.06875 12 0.838149C12 0.771011 12 0.703873 12 0.633816Z"
                                                                fill="currentColor" />
                                                        </svg>
                                                    </button>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="cart-price">
                                            <p className="cart-pera text-center">$110</p>
                                        </td>
                                        <td className="action">
                                            <div className="text-center">
        
                                                <a href="#" className="del-icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="21"
                                                        viewBox="0 0 20 21" fill="none">
                                                        <path
                                                            d="M9.76126 20.9806C8.58921 20.9291 7.11625 21.0958 5.65575 20.9082C4.14925 20.7177 3.06824 19.5157 2.97241 18.0042C2.78074 15.0954 2.58908 12.186 2.39741 9.27591C2.30253 7.90438 2.19424 6.52618 2.11662 5.14989C2.09841 4.8194 1.97382 4.69558 1.65278 4.71082C1.36541 4.72641 1.07733 4.72386 0.790281 4.7032C0.336988 4.66605 0.0331962 4.36412 0.00252952 3.95076C-0.0290955 3.50692 0.237322 3.1707 0.717448 3.09356C0.957043 3.06347 1.19866 3.05232 1.44003 3.06022C2.81237 3.06022 4.18566 3.03641 5.56087 3.06022C6.03142 3.06879 6.18667 2.90973 6.14737 2.45827C6.11989 2.17394 6.13637 1.88711 6.19625 1.60773C6.40612 0.676239 7.21496 0.0447651 8.26242 0.0200014C9.41242 -0.00666715 10.5624 -0.00666715 11.7124 0.0200014C13.0359 0.0523847 13.8639 0.927686 13.8687 2.24492V2.38779C13.8687 3.05451 13.8687 3.05451 14.5165 3.05451L18.7332 3.06308C18.9086 3.057 19.0843 3.06528 19.2584 3.08784C19.7423 3.16594 20.0174 3.48692 19.9992 3.92885C19.9809 4.37079 19.6714 4.67367 19.1903 4.70796C18.9031 4.72546 18.6152 4.72768 18.3278 4.71462C18.0288 4.7032 17.9061 4.82035 17.8889 5.12894C17.7324 7.86628 17.5682 10.603 17.3963 13.3391C17.2957 14.9516 17.2439 16.5717 17.0504 18.1737C16.8424 19.8986 15.5448 20.9596 13.792 20.9806C12.5462 20.9929 11.3013 20.9806 9.76126 20.9806ZM9.28209 4.71748H4.39554C3.78987 4.71748 3.76879 4.71748 3.80233 5.32705C3.91829 7.39863 4.05054 9.47021 4.17608 11.5418C4.3045 13.6762 4.4262 15.8116 4.56516 17.9461C4.57959 18.3168 4.73856 18.6674 5.00846 18.9237C5.27835 19.1801 5.63799 19.322 6.01129 19.3195C8.67674 19.3328 11.3419 19.3328 14.0067 19.3195C14.3714 19.3142 14.7207 19.1724 14.9849 18.9224C15.249 18.6724 15.4086 18.3326 15.4317 17.9708C15.6017 15.57 15.7614 13.1673 15.9109 10.7627C16.0259 8.94445 16.1026 7.12337 16.2492 5.308C16.2856 4.84797 16.1399 4.7813 15.7413 4.76987C13.586 4.70701 11.4316 4.72415 9.27826 4.71748H9.28209ZM9.99988 3.04689C10.5912 3.04689 11.1815 3.04689 11.7718 3.04689C12.0478 3.04689 12.3037 3.04212 12.2654 2.65829C12.2299 2.30493 12.4111 1.77918 11.8418 1.75632C10.6151 1.70679 9.38367 1.70679 8.157 1.75632C7.58871 1.77918 7.77367 2.30588 7.73438 2.65924C7.69604 3.04022 7.95288 3.04879 8.22888 3.04689C8.81921 3.04689 9.40955 3.04689 9.99988 3.04689Z"
                                                            fill="currentColor" />
                                                        <path
                                                            d="M6.13672 11.978C6.13672 10.6932 6.13672 9.40643 6.13672 8.12253C6.13672 7.51296 6.44818 7.14246 6.93789 7.14246C7.4276 7.14246 7.73235 7.52344 7.73235 8.13205C7.73235 10.7183 7.73235 13.3045 7.73235 15.8907C7.73235 16.506 7.4343 16.8803 6.95226 16.8936C6.47022 16.907 6.13672 16.5174 6.13672 15.8812C6.13672 14.5801 6.13672 13.2791 6.13672 11.978Z"
                                                            fill="currentColor" />
                                                        <path
                                                            d="M9.20312 11.9714C9.20312 10.7015 9.20312 9.43155 9.20312 8.16162C9.20312 7.51395 9.50979 7.13678 10.0129 7.1444C10.4998 7.15107 10.8007 7.52538 10.8007 8.1359C10.8007 10.7228 10.8007 13.3093 10.8007 15.8955C10.8007 16.5079 10.4978 16.8832 10.0148 16.8918C9.53183 16.9003 9.20504 16.5108 9.20312 15.8726C9.20312 14.5716 9.20312 13.2725 9.20312 11.9714Z"
                                                            fill="currentColor" />
                                                        <path
                                                            d="M13.8668 12.0686C13.8668 13.3861 13.8668 14.703 13.8668 16.0193C13.8668 16.4565 13.6626 16.7518 13.241 16.867C12.8931 16.9622 12.618 16.8099 12.4101 16.5327C12.3045 16.3724 12.2553 16.1818 12.2702 15.9908C12.2702 13.341 12.2702 10.6904 12.2702 8.04066C12.2702 7.50443 12.595 7.15202 13.057 7.14631C13.5189 7.14059 13.862 7.50633 13.8639 8.07018C13.8706 9.40266 13.8668 10.7361 13.8668 12.0686Z"
                                                            fill="currentColor" />
                                                    </svg>
                                                </a>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td className="cart-img">
                                            <div className="thumb-img">
                                                <img src="/assets/images/gallery/wish-thumb-4.png" alt="img" />
                                            </div>
                                        </td>
                                        <td className="cart-info">
                                            <div className="cart-box">
                                                <a href="/shop-details">
                                                    <p className="cart-pera mb-15">Otello Armchair Ghini</p>
                                                </a>
                                                <div className="ratting-section mb-18">
                                                    <div className="all-ratting">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14"
                                                            viewBox="0 0 15 14" fill="none">
                                                            <path
                                                                d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                                                fill="#FFA800" />
                                                        </svg>
                                                    </div>
                                                    <div className="ratting-count">
                                                        <p className="pera">(41)</p>
                                                    </div>
                                                </div>
                                                <p className="cart-pera">$640</p>
                                            </div>
                                        </td>
                                        <td className="cart-qty">
                                            <div className="quantity-section">
                                                <div className="quantity-btn position-relative">
                                                    <input type="text" name="qty" value="01" className="num-count" readonly />
                                                    <button className="increase-num">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="12" height="6"
                                                            viewBox="0 0 12 6" fill="none">
                                                            <path
                                                                d="M0 5.36618C0.0532405 5.553 0.143084 5.71647 0.319443 5.84199C0.618921 6.04924 1.0515 6.05508 1.35098 5.84491C1.40755 5.80404 1.46411 5.76026 1.51735 5.71355C2.97481 4.43501 4.4356 3.15355 5.89306 1.875C5.92633 1.84581 5.95295 1.81078 6.03281 1.75824C6.0561 1.79327 6.06941 1.83705 6.10269 1.86625C7.57346 3.15938 9.04755 4.4496 10.5183 5.74274C10.7779 5.97043 11.0773 6.058 11.4367 5.95875C11.9591 5.8128 12.1721 5.22315 11.8427 4.83784C11.8061 4.79405 11.7628 4.75318 11.7196 4.71232C10.0225 3.2236 8.32881 1.73489 6.63177 0.249093C6.32231 -0.0223789 5.92966 -0.0749219 5.57361 0.106059C5.4871 0.149845 5.41056 0.214063 5.34069 0.275363C3.69356 1.72029 2.04976 3.16522 0.399304 4.60723C0.216289 4.76778 0.0532405 4.93125 0 5.16185C0 5.22899 0 5.29613 0 5.36618Z"
                                                                fill="currentColor" />
                                                        </svg>
                                                    </button>
                                                    <button className="decrease-num">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="12" height="6"
                                                            viewBox="0 0 12 6" fill="none">
                                                            <path
                                                                d="M12 0.633816C11.9468 0.446997 11.8569 0.28353 11.6806 0.158011C11.3811 -0.0492418 10.9485 -0.0550799 10.649 0.155092C10.5925 0.195958 10.5359 0.239744 10.4826 0.286449C9.02519 1.56499 7.5644 2.84645 6.10694 4.125C6.07367 4.15419 6.04705 4.18922 5.96719 4.24176C5.9439 4.20673 5.93059 4.16294 5.89731 4.13375C4.42654 2.84062 2.95245 1.5504 1.48168 0.257257C1.22213 0.0295717 0.922652 -0.0579998 0.563279 0.0412478C0.0408569 0.1872 -0.172105 0.776848 0.157321 1.16216C0.193924 1.20595 0.237181 1.24681 0.280439 1.28768C1.97748 2.7764 3.67119 4.26511 5.36823 5.75091C5.67769 6.02238 6.07034 6.07492 6.42639 5.89394C6.5129 5.85015 6.58944 5.78594 6.65931 5.72464C8.30644 4.27971 9.95024 2.83478 11.6007 1.39277C11.7837 1.23222 11.9468 1.06875 12 0.838149C12 0.771011 12 0.703873 12 0.633816Z"
                                                                fill="currentColor" />
                                                        </svg>
                                                    </button>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="cart-price">
                                            <p className="cart-pera text-center">$640</p>
                                        </td>
                                        <td className="action">
                                            <div className="text-center">
        
                                                <a href="#" className="del-icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="21"
                                                        viewBox="0 0 20 21" fill="none">
                                                        <path
                                                            d="M9.76126 20.9806C8.58921 20.9291 7.11625 21.0958 5.65575 20.9082C4.14925 20.7177 3.06824 19.5157 2.97241 18.0042C2.78074 15.0954 2.58908 12.186 2.39741 9.27591C2.30253 7.90438 2.19424 6.52618 2.11662 5.14989C2.09841 4.8194 1.97382 4.69558 1.65278 4.71082C1.36541 4.72641 1.07733 4.72386 0.790281 4.7032C0.336988 4.66605 0.0331962 4.36412 0.00252952 3.95076C-0.0290955 3.50692 0.237322 3.1707 0.717448 3.09356C0.957043 3.06347 1.19866 3.05232 1.44003 3.06022C2.81237 3.06022 4.18566 3.03641 5.56087 3.06022C6.03142 3.06879 6.18667 2.90973 6.14737 2.45827C6.11989 2.17394 6.13637 1.88711 6.19625 1.60773C6.40612 0.676239 7.21496 0.0447651 8.26242 0.0200014C9.41242 -0.00666715 10.5624 -0.00666715 11.7124 0.0200014C13.0359 0.0523847 13.8639 0.927686 13.8687 2.24492V2.38779C13.8687 3.05451 13.8687 3.05451 14.5165 3.05451L18.7332 3.06308C18.9086 3.057 19.0843 3.06528 19.2584 3.08784C19.7423 3.16594 20.0174 3.48692 19.9992 3.92885C19.9809 4.37079 19.6714 4.67367 19.1903 4.70796C18.9031 4.72546 18.6152 4.72768 18.3278 4.71462C18.0288 4.7032 17.9061 4.82035 17.8889 5.12894C17.7324 7.86628 17.5682 10.603 17.3963 13.3391C17.2957 14.9516 17.2439 16.5717 17.0504 18.1737C16.8424 19.8986 15.5448 20.9596 13.792 20.9806C12.5462 20.9929 11.3013 20.9806 9.76126 20.9806ZM9.28209 4.71748H4.39554C3.78987 4.71748 3.76879 4.71748 3.80233 5.32705C3.91829 7.39863 4.05054 9.47021 4.17608 11.5418C4.3045 13.6762 4.4262 15.8116 4.56516 17.9461C4.57959 18.3168 4.73856 18.6674 5.00846 18.9237C5.27835 19.1801 5.63799 19.322 6.01129 19.3195C8.67674 19.3328 11.3419 19.3328 14.0067 19.3195C14.3714 19.3142 14.7207 19.1724 14.9849 18.9224C15.249 18.6724 15.4086 18.3326 15.4317 17.9708C15.6017 15.57 15.7614 13.1673 15.9109 10.7627C16.0259 8.94445 16.1026 7.12337 16.2492 5.308C16.2856 4.84797 16.1399 4.7813 15.7413 4.76987C13.586 4.70701 11.4316 4.72415 9.27826 4.71748H9.28209ZM9.99988 3.04689C10.5912 3.04689 11.1815 3.04689 11.7718 3.04689C12.0478 3.04689 12.3037 3.04212 12.2654 2.65829C12.2299 2.30493 12.4111 1.77918 11.8418 1.75632C10.6151 1.70679 9.38367 1.70679 8.157 1.75632C7.58871 1.77918 7.77367 2.30588 7.73438 2.65924C7.69604 3.04022 7.95288 3.04879 8.22888 3.04689C8.81921 3.04689 9.40955 3.04689 9.99988 3.04689Z"
                                                            fill="currentColor" />
                                                        <path
                                                            d="M6.13672 11.978C6.13672 10.6932 6.13672 9.40643 6.13672 8.12253C6.13672 7.51296 6.44818 7.14246 6.93789 7.14246C7.4276 7.14246 7.73235 7.52344 7.73235 8.13205C7.73235 10.7183 7.73235 13.3045 7.73235 15.8907C7.73235 16.506 7.4343 16.8803 6.95226 16.8936C6.47022 16.907 6.13672 16.5174 6.13672 15.8812C6.13672 14.5801 6.13672 13.2791 6.13672 11.978Z"
                                                            fill="currentColor" />
                                                        <path
                                                            d="M9.20312 11.9714C9.20312 10.7015 9.20312 9.43155 9.20312 8.16162C9.20312 7.51395 9.50979 7.13678 10.0129 7.1444C10.4998 7.15107 10.8007 7.52538 10.8007 8.1359C10.8007 10.7228 10.8007 13.3093 10.8007 15.8955C10.8007 16.5079 10.4978 16.8832 10.0148 16.8918C9.53183 16.9003 9.20504 16.5108 9.20312 15.8726C9.20312 14.5716 9.20312 13.2725 9.20312 11.9714Z"
                                                            fill="currentColor" />
                                                        <path
                                                            d="M13.8668 12.0686C13.8668 13.3861 13.8668 14.703 13.8668 16.0193C13.8668 16.4565 13.6626 16.7518 13.241 16.867C12.8931 16.9622 12.618 16.8099 12.4101 16.5327C12.3045 16.3724 12.2553 16.1818 12.2702 15.9908C12.2702 13.341 12.2702 10.6904 12.2702 8.04066C12.2702 7.50443 12.595 7.15202 13.057 7.14631C13.5189 7.14059 13.862 7.50633 13.8639 8.07018C13.8706 9.40266 13.8668 10.7361 13.8668 12.0686Z"
                                                            fill="currentColor" />
                                                    </svg>
                                                </a>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr className="cart-button">
                                        <td colspan="3">
                                            <a href="/shop" className="outline-pill-btn">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="78" height="19"
                                                    viewBox="0 0 78 19" fill="none">
                                                    <path
                                                        d="M10.4307 8.08519C10.45 8.00902 10.4733 7.93404 10.5005 7.86057C11.0144 6.48086 11.4919 5.08506 11.9655 3.69049C12.0652 3.39928 12.3056 3.10909 12.0209 2.67218C8.41924 5.58385 4.25553 7.51216 0 9.34863C4.377 11.2685 8.53825 13.4042 12.2683 16.3829C12.4931 15.9325 12.2509 15.6395 12.1415 15.3422C11.6228 13.9377 11.1054 12.5293 10.5522 11.141C10.1633 10.1548 10.1204 9.0761 10.4307 8.08519Z"
                                                        fill="currentColor" />
                                                    <rect x="77.3496" y="10.0935" width="68" height="1" rx="0.5"
                                                        transform="rotate(-180 77.3496 10.0935)" fill="currentColor" />
                                                </svg>
                                                Continue Shopping
                                            </a>
                                        </td>
                                        <td colspan="2">
                                            <div className="text-right">
                                                <a href="#" className=" cart-btn">Update Cart</a>
                                            </div>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>						
					</div>
				</div>
            </div>

        </div>
        

        
        <section className="all-mini-card bottom-padding1">
            <div className="container">
                <div className="row g-4">
                    <div className="col-xl-4 col-md-6">
                        <div className="mini-card h-100">
                            <div className="header">
                                <h4 className="title">Coupon Code</h4>
                            </div>
                            <div className="body">
                                <p className="pera">There are many variations of passages of Lorem Ipsumis
                                    suffered alteration in some form, by injected huvmour, or
                                    look even slightly believable.</p>
                                <div className="formbar">
                                    <input type="text" className="custom-form-control" placeholder="Coupon Code" />
                                </div>
                                <div className="button-section">
                                    <a href="#" className="cart-btn">Apply Coupon</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="col-xl-4 col-md-6">
                        <div className="mini-card h-100">
                            <div className="header">
                                <h4 className="title">Calculate Shipping</h4>
                            </div>
                            <div className="body">
                                <div className="mb-20">
                                    <div className="country-dropdown">
                                        <select name="country" className="select2">

                                        </select>
                                    </div>
                                </div>
                                <div className="mb-20">
                                    <input type="text" className="custom-form-control" placeholder="State" />
                                </div>
                                <div className="mb-20">
                                    <input type="text" className="custom-form-control" placeholder="Town / City" />
                                </div>
                                <div className="mb-20">
                                    <input type="text" className="custom-form-control" placeholder="Postcode / ZIP" />
                                </div>
                                <div className="button-section">
                                    <a href="#" className="cart-btn">Update</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="col-xl-4 col-md-6">
                        <div className="mini-card h-100">
                            <div className="header">
                                <h4 className="title">Cart Totals</h4>
                            </div>
                            <div className="body">
                                <div className="price-list">
                                    <h4 className="title text-color-primary">Subtotal</h4>
                                    <div className="title text-color-tertiary">$3,540</div>
                                </div>
                                <div className="divider"></div>
                                <div className="price-list">
                                    <h4 className="title text-color-primary">Shipping</h4>
                                    <div className="title text-color-tertiary">Free Shipping</div>
                                </div>
                                <div className="price-list">
                                    <h4 className="title text-color-primary">Tax</h4>
                                    <div className="title text-color-tertiary">$00</div>
                                </div>
                                <div className="price-list">
                                    <h4 className="title text-color-primary">Local Delivery</h4>
                                    <div className="title text-color-tertiary">$20</div>
                                </div>
                                <div className="divider"></div>
                                <div className="price-list">
                                    <h4 className="title text-color-primary font-600">Total</h4>
                                    <div className="title text-color-tertiary">$3,560</div>
                                </div>
                                <div className="button-section">
                                    <a href="/checkout" className="cart-btn">Checkout</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </section>
        
        
        <section className="subscription-area subscription-bg">
            <div className="container">
                <div className="row">
					<div className="col-lg-12">
                        <div className="subscription-wrapper">
                            <div className="left-wrapper">
                                <div className="subscription-content">
                                    <h4 className="title">Get a surprise discount</h4>
                                    <p className="pera">Join our email subscription now</p>
                                </div>
                                <div className="subscription-input-section">
                                    <input type="text" className="subscription-input" placeholder="Enter your email address" />
                                    <button type="submit" className="subscribe-btn"><span className="btn-text">Subscribe</span><span
                                            className="icon"><i className="ri-arrow-right-line"></i></span></button>
                                </div>
                            </div>
                            <div className="right-wrapper">
                                <div className="subscription-content">
                                    <h4 className="title">Download App</h4>
                                    <p className="pera">Save $3 With App & New User only</p>
                                </div>
                                <div className="download-app">
                                    <a href="#" target="_blank">
                                        <img src="/assets/images/icon/google-play.png" alt="img" />
                                    </a>
                                    <a href="#" target="_blank">
                                        <img src="/assets/images/icon/app-store.png" alt="img" />
                                    </a>
                                </div>
                            </div>
                        </div>						
					</div>
				</div>
            </div>
        </section>
        
    </main>
    
    
      <Footer />
      <ScrollTop />
    </>
  )
}

export default Cart
